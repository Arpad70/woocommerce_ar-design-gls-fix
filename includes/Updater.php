...
final class ArDesignGlsFixUpdater
{
    ...
    {
        if ('plugin_information' !== $action || ! is_object($args) || ! isset($args->slug) || 'ar-design-gls-fix' !== $args->slug) {
            return $result;
        }
        ...
        return (object) array(
            'name' => 'AR Design GLS Fix for WooCommerce',
            'slug' => 'ar-design-gls-fix',
            'version' => $version,
            ...
            'sections' => array(
                'description' => __('Samostatný GLS fix modul pre WooCommerce spravovaný AR Design.', 'ar-design-gls-fix'),
                'changelog' => '' !== $body ? wp_kses_post(nl2br(esc_html($body))) : __('Changelog nie je dostupný.', 'ar-design-gls-fix'),
            ),
        );
    }
        ...
        $response = wp_remote_get(
            ...
            array(
                ...
                'headers' => array(
                    ...
                    'User-Agent' => 'ar-design-gls-fix/' . $this->currentVersion,
                ),
            )
        );
    ...
    {
        return 'ar_design_gls_fix_release_data_' . md5($this->repositoryFullName);
    }
}
